...
model.thresh = min(model.thresh,-2);
boxes_gtbox0 = testmodel_gtbox(name,model,test,suffix);



%%%% data augmentation
[test1 test2 test3 test4 aug_num]=data_augmentation(test);
%%%% more test results
boxes_gtbox1 = testmodel_gtbox(name,model,test1,suffix);
boxes_gtbox2 = testmodel_gtbox(name,model,test2,suffix);
boxes_gtbox3 = testmodel_gtbox(name,model,test3,suffix);
boxes_gtbox4 = testmodel_gtbox(name,model,test4,suffix);
boxes_gtbox=boxes_gtbox0;
for i=1:length(boxes_gtbox0)
    %boxes1{i}(:,1:end-2)=boxes1{i}(:,1:end-2)+10;
    boxes_gtbox{i}=(boxes_gtbox0{i}+boxes_gtbox1{i}+boxes_gtbox2{i}+boxes_gtbox3{i}+boxes_gtbox4{i})/5;
end
fprintf('the test stage is done, and we have got the average result.\n');
